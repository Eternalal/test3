...
public class JDBCutil {
    ...
    // 数据库连接字符串
    private String conStr = "jdbc:mysql:///springboot_test?Timezone=UTC&Unicode=true&characterEnconding=utf-8";//最新版本的mysql驱动
    // 数据库连接用户名
    private String dbUserName = "root";
    // 数据库连接密码
    private String dbPassword = "Demonsgudu123";
    // 数据库连接对象
    ...
    public static boolean addUserInfo(Person contactPerson) {
        String sql = "insert into p_user (id,name ,phone,address) values (?,?,?,?)";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    public static boolean modifyUserInfo(Person contactPerson) {
        String sql = "update p_user set name=?,phone=?,address=? where id=?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
